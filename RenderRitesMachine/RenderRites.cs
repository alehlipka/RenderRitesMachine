...
public sealed class RenderRites : IDisposable
{
    ...
    private RenderRites()
    {
        ...
        var sceneFactory =
            new SceneFactory(AssetsService, TimeService, RenderService, AudioService, GuiService, Logger);
    ...
    public void RunWindow(string? title)
    {
        ...
        Logger.LogInfo(
            $"Starting RenderRites window: '{title}' ({RenderConstants.DefaultWindowWidth}x{RenderConstants.DefaultWindowHeight})");
